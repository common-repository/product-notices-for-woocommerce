...
class CRWCPN_Admin {
	...
	public static function get_settings() {
		...
		return array(
			...
			array(
				...
				'type'  => 'title',
				'desc'  => ' ',	// phpcs:ignore
				'id'    => 'crwcpn-display-warning-notice',
			),
			...
			/**
			 * Border radius options allow you to control the amount of curvature for the corners of an element.
			 *
			 * @since 1.3.0
			 */
			array(
				'id'          => 'crwcpn_product_notice_custom_border_radius',
				'name'        => __( 'Border Radius', 'product-notices-woocommerce' ),
				'type'        => 'number',
				'placeholder' => 'px',
				'desc'        => __( 'This will be used as the border radius for the notice box.', 'product-notices-woocommerce' ),
				'desc_tip'    => true,
			),
	...
	public static function features_widget() {
		$args = array(
			...
			'link'         => 'https://docs.cloudredux.com/feed/?post_type=cr-plugin-features&cr_plugins=product-notices-for-woocommerce',
			'url'          => 'https://docs.cloudredux.com/feed/?post_type=cr-plugin-features&cr_plugins=product-notices-for-woocommerce',
	 ...
	 * @since 1.3.0
	 */
	public static function crwcpn_get_all_products() {
		$orderby = 'name';
		$order   = 'asc';
		...
		$crwcpn_products = wc_get_products( $pro_args );

		$crwcpn_woo_products = array();
		if ( ! empty( $crwcpn_products ) && ! is_wp_error( $crwcpn_products ) ) {
			foreach ( $crwcpn_products as $product ) {
				$crwcpn_woo_products[ $product->get_id() ] = $product->get_title();
			}
		}

		return apply_filters( 'crwcpn_products', $crwcpn_woo_products );
	}
}
